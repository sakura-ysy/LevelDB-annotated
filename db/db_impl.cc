...
DBImpl::DBImpl(const Options& raw_options, const std::string& dbname)
    : 
      ...
      // 设置options:
      //     1. 参数. 2. info_log 3. block_cache
      options_(SanitizeOptions(dbname, &internal_comparator_,
                               &internal_filter_policy_, raw_options)),
      // 是否有options中的info_log
      owns_info_log_(options_.info_log != raw_options.info_log),
      // 是否有options中的block_cache
      ...
      // 是否有一个后台Compact线程正在被调度或是运行中
      background_compaction_scheduled_(false),
      // 手动的Compact
      manual_compaction_(nullptr),
      // 生成一个空的VersionSet
      versions_(new VersionSet(dbname_, &options_, table_cache_,  // DBImpl 根据 options 新建 VersionSet
...
// 新建DB，实际就是新建DB的Manifest和指向它的CURRENT文件。
// 并不是返回一个DB对象！
// 初始化的信息（next_file_number等等）暂存入VersionEdit中，也就是最初始的VersionEdit，
// 然后写入Manifest，待到VersionSet::Recover()时将该初始VersionEdit应用到VersionSet中去。
// 
Status DBImpl::NewDB() {
  // 新建一个（原始）VersionEdit
  // 和序号有关的初始化信息先`暂存`其中
  ...
  // 接下来把这个新DB的初始化信息（原始VersionEdit）通过写WAL日志的方式写入Manifest。
  // Descriptor就是Manifest，number为1
  const std::string manifest = DescriptorFileName(dbname_, 1); // Manifest文件名
  WritableFile* file; // Manifest文件句柄
  // Manifest文件结构与WAL文件相同
  // 这里创建文件并生成相应句柄
  Status s = env_->NewWritableFile(manifest, &file);
  if (!s.ok()) {
    return s;
  }
  // 把该VersionEdit写入Manifest中
  {
    log::Writer log(file); // 封装成Writer
    std::string record;
    // 将原始VersionEdit编码后写入Manifest
  ...
  // Manifest建完了，就要建CURRENT了
  if (s.ok()) {
    // Make "CURRENT" file that points to the new manifest file.
    // Manifest写入成功，那么新建CURRENT文件指向它
...
Status DBImpl::Recover(VersionEdit* edit, bool* save_manifest) {
  ...
  // 因为刚创建DB，所以DB相关的文件锁变量肯定还不存在
  assert(db_lock_ == nullptr);
  // 创建文件锁，并让变量db_lock_指向它
  ...
  // 是否需要创建新的DB由两个条件来决定:
  // 1. ${dbname_}/CURRENT文件是否存在
  // 2. create_if_missing是否需要创建
  // 如果该dbname_的CURRENT文件不存在，则说明该DB不存在，看是否要新建
  if (!env_->FileExists(CurrentFileName(dbname_))) {
    if (options_.create_if_missing) {
      Log(options_.info_log, "Creating DB %s since it was missing.",
          dbname_.c_str());
      // 如果是一个新的DB，那么CURRENT肯定是不存在的，故新建。
      s = NewDB(); // 这里只是新建Manifest和CURRENT文件，没有新建DB对象。
      if (!s.ok()) {
        return s;
      }
    } else {
  ...
  // 到这一步，无论是已有的CURRENT，还是新建的CURRENT，该文件都是存在的
...
Status DB::Open(const Options& options, const std::string& dbname, DB** dbptr) {
  *dbptr = nullptr;
  ...
  // LevelDB并没有将DB不存在和DB已存在通过不同的方式来处理,
  // 这两个逻辑都是通过Recovery进行统一的。
